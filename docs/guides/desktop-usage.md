...
- Export format selection writes JSON, CSV, SVG, PNG, or a self-contained HTML workbook report from the current filtered view. HTML also includes the selected Session's full call detail when a session is open.
