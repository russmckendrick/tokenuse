...
v10 extends that guarded pass for the September 22, 2026 launches and re-runs it once over full history. Claude Opus 5.5 ids inherited the Opus 5 row through longest-prefix matching, so Claude Code and Cursor rows were charged $5/$25 with 0.1x cache reads instead of $4/$20 with 0.05x. The buckets do not share one factor, so the repair recovers the hidden 1-hour cache-write count from the exact Opus 5 hypothesis and recomputes under Opus 5.5, dividing out and re-applying the 2x fast multiplier both rows carry. GPT-6 Sol and Luna Codex rows that exactly match the Sonnet fallback formula are recomputed at their official rates, alongside GPT-6 Astra. Re-running the whole chain also catches Astra rows in archives that reached v9 before the Astra repair joined it; every check proves the stale formula first, so rows that are already correct stay untouched.
