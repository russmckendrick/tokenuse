# Unreleased

Changes that should be included in the next release go here. Keep this file current during normal development; move the relevant notes into `docs/releases/<version>.md` only when preparing a release.

## Highlights

- Fixed desktop filter dropdowns so expanded menus stay above dashboard panel title bars.
- Centralized TUI and desktop in-window keyboard shortcuts in a shared embedded keymap so behavior, help text, and footer hints stay in sync.
- Moved bundled sample dashboard data into anonymized embedded JSON, rebased sample dates around today, converted sample Usage costs with the active currency, and added `Shift-D` to toggle between live and sample data.
- Added a self-contained HTML workbook export for TUI and desktop with dashboard panels, inline Token Use branding, and selected Session full call detail when a session is open.

## Notes

- Version bumps and numbered release notes are release-prep tasks, not part of every user-visible change.
