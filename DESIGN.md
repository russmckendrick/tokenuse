...
- **Desktop track** — the Tauri + Svelte app. A modern sidebar-rail analytics application with more screens and more room: Overview, Analytics, Graph, Coach, Scrollback, Tools, Models, Projects, and Config, each a scrollable page. Graphs and utilisation are the hero; provider icons identify models at a glance. The desktop is not a terminal copy — it shares the palette, the brand mark, and the data-dense table DNA, and diverges everywhere a desktop app earns the space.
...
- Primary items — Overview, Analytics, Graph, Coach, Scrollback, Models, Projects — each a 32px row: 16px icon, 13px Inter label, `rounded.sm` hover tint, primary-colored active indicator (2px inset bar on the left edge).
...
Nine screens. Data pages fetch page-scoped queries from the core (memoized per filter set); the 3-second snapshot poll carries only the shared dashboard, limits, and filter state.
...
- **Graph** — a full-width relationship explorer with Projects and AI-stack lenses. Period/tool/project filters scope the graph; Calls/Spend/Tokens controls weight and bound it. Project, desktop-tool, and canonical-model nodes form the default map; Core tools and MCP servers are opt-in layers. Search, zoom/pan/drag, persistent selection, and the side inspector lead to exact project/model/tool detail without exposing prompts, sessions, files, or shell commands.
...
- **Scrollback** — local full-text transcript search with tool/project filters, grouped session results, highlighted snippets, and session drill-in. Search owns its page toolbar and keeps its query/results across the session round trip.
...
- **Relationship graph** — a WebGL 3D force layout, rendered by `3d-force-graph` and Three.js, with deterministic starts and type-specific depth planes. Projects, AI tools, models, Core tools, and MCP servers use distinct token-coloured geometries; node radius and edge width use square-root/log-like scaling against the selected Calls/Spend/Tokens measure. The strongest anchor labels remain visible while dense satellites reveal labels on hover, search, or selection. Selecting a node dims non-neighbours, strengthens its exact connections, and drives a persistent inspector. The canvas supports orbit, zoom, node drag, graph-aware fit, and reset; reduced motion settles before first paint. Entity and relationship caps are disclosed rather than silently implying completeness.
