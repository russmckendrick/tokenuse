...
- For dependency changes and every release, audit both Rust lockfiles from the repo root with `cargo audit --file Cargo.lock` and `cargo audit --file desktop/src-tauri/Cargo.lock`, then run `CI=true pnpm audit --audit-level low` from `desktop/`. Use cargo-audit 0.22.2 to match CI and fetch current advisory data; include development dependencies. Audit service/network failures are failed checks, not clean results. Review and report advisory warnings as well as vulnerabilities; do not add ignores or weaken the audit threshold just to publish.
...
- The dashboard reads usage files directly - no Anthropic/OpenAI platform API keys, no proxy, no telemetry. Don't add network calls outside explicit Config-page downloads or maintainer refresh feature paths. The one inbound exception is the opt-in MCP HTTP endpoint (`tokenuse mcp --http`, desktop Config toggle): loopback-only, bearer-token-gated, serves local data to local clients, and makes no outbound calls - see `src/mcp/http.rs` and `docs/development/architecture.md` before touching it. The opt-in `quota-sync` feature stores a Claude.ai or ChatGPT session cookie locally (OS keychain) and calls those services' user-facing usage endpoints on explicit Config-page sync and, once a provider is opted in, from the 15-minute background refresher. Copilot's opt-in artifact is its sidecar file rather than a cookie - deleting `limits/copilot.json` stops its auto-refresh. See `docs/development/tools/claude-subscription.md`, `docs/development/tools/codex-subscription.md`, and `docs/development/tools/copilot.md` before adjusting that flow.
...
- Before tagging a release, run all root and desktop checks above, even for a small release, plus both Rust audits and the frontend audit. Keep the root Cargo package, desktop Cargo package, `desktop/package.json`, and `desktop/src-tauri/tauri.conf.json` versions aligned, and regenerate both Cargo lockfiles through Cargo. Confirm the final commit passes the root/desktop CI matrix and audit jobs before pushing its tag. Release builds depend on the audits, and the release workflow publishes the checked-in release title and notes. Verify the published assets, updater manifest, and distribution jobs after the tag finishes.
